...
class EnrollmentViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ['create']:
            ...
            return [IsEnrolledStudent() if self.request.user.role == 'STUDENT' else IsTeacher()] 
        return []
...
class CourseFileViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.request.method in SAFE_METHODS: 
            ...
            return [IsAuthenticated(), IsEnrolledStudent() self.request.user.role == 'STUDENT' else IsCourseTeacher() ]
        else:
