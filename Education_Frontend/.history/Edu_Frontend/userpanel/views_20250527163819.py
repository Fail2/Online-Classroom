...
def download_file(request,file_id):
    ...
    file_url = f'http://127.0.0.1:8000/api/courses/CourseFiles/{file_id}/'
    response = api_request_with_refresh(request,'GET',file_url)
    if isinstance(response,HttpResponse):
        return response
    if response.status_code == 200:
        file_data=response.json()
    else:
        return HttpResponse(f'File not found. Status: {response.status_code},{response.text}')
    print(file_data)
    file_url = file_data['file']

    # Step 2: Fetch the actual file content
    files_response = requests.get(file_url)
    if files_response.status_code!=200:
        return HttpResponse('Error downloadinf file', status =400)
    filename = file_url.split("/")[-1]

    # Step 3: Return file as downloadable response
    response = HttpResponse(files_response.content,content_type = 'application/octet-stream')
