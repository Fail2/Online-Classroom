...
def download_file(request, file_id):
    ...
    file_url = f'http://127.0.0.1:8000/api/courses/CourseFiles/{file_id}/download/'

    # Make authenticated request using your helper
    response = api_request_with_refresh(request, 'GET', file_url, stream=True)

    # If authentication failed, return the error page
    if isinstance(response, HttpResponse):
        return response
    ...
    if response.status_code == 200:
        ...
        return HttpResponse(f'Download failed. Status: {response.status_code}, {response.text}')
